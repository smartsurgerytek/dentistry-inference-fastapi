...
def init_context(context):
    ...
    context.user_data.model_handler = YOLO('/opt/nuclio/pano_model.pt')
...
def transform(image, model):
    ...
    for box, conf, mask in zip(boxes, confs, masks.data):
        ...
        # Properly resize mask to match image dimensions
        mask_np = mask.cpu().numpy()
        mask_np = cv2.resize(mask_np, (img_width, img_height), interpolation=cv2.INTER_LINEAR)
