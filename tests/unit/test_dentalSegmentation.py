...
def test_dentalSegmentation_normalImage():
    ...
    if not results_dict.get('Dentin') is not None:
        raise ValueError("When input normal image, the result should be found, plz check dental_segmentation function dict option")
    results_dict=yolo_transform(image, model, return_type='yolov8')
    if not results_dict.get('yolov8_contents'):
        raise ValueError("When input normal image, the result should be found, plz check dental_segmentation function yolov8 option")   
