...
def restructure_dataframe(df):
    ...
    df_mapping = {'CEJ': "enamel", 'ALC': "dentin", 'APEX': "gum"}
    df_list = [pd.json_normalize(measurement[0]) for measurement in df['pair_measurements']]
    df_final = pd.concat(df_list, ignore_index=True)
    df_rename=df_final.rename(columns=df_mapping)
    df_rename['tooth_id'] = range(1, len(df) + 1)
...
def test_detntalMeasure_performance():
    ...
    for folder_name in os.listdir(main_folder_path):
        ...
        if df_pred.empty:
            ...
            print(df_merged['predicted_stage'], df_merged['true_stage'])
        breakpoint()
    value_mapping = {
        "0": 0,
        "I": 1,
